    ...
    return (
      <div className="grid min-h-[360px] place-items-center rounded-3xl border border-[#0879D9]/15 bg-white">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-slate-600">Đang tải cài đặt thương hiệu...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="font-vietnamese space-y-6">
      <header className="rounded-3xl border border-[#0879D9]/15 bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-extrabold text-[#071827]">Cài đặt thương hiệu OMFIT</h2>
          ...
          <button
            ...
            disabled={isSaving}
            className="gradient-bg-omfit-btn inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isSaving
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-3xl border border-[#0879D9]/15 bg-white p-5 sm:p-6">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h3 className="text-base font-extrabold text-[#071827]">Hướng dẫn và tài sản thương hiệu</h3>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => logoInputRef.current?.click()} disabled={isUploading !== null} className="min-h-28 rounded-2xl border-2 border-dashed border-[#0879D9]/30 bg-[#F8FAFC] p-4 text-left transition hover:border-[#0879D9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0879D9] disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <span className="mt-2 block text-sm font-extrabold text-[#071827]">{isUploading === 'logo' ? 'Đang tải logo...' : 'Tải logo lên'}</span>
            ...
            <button type="button" onClick={() => guidelineInputRef.current?.click()} disabled={isUploading !== null} className="min-h-28 rounded-2xl border-2 border-dashed border-[#0879D9]/30 bg-[#F8FAFC] p-4 text-left transition hover:border-[#0879D9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0879D9] disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <span className="mt-2 block text-sm font-extrabold text-[#071827]">{isUploading === 'guideline' ? 'Đang tải tài liệu...' : 'Tải hướng dẫn lên'}</span>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 p-3">
              <p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Tài liệu hướng dẫn</p>
          ...
          {draft.branches.map((branch, index) => (
            <fieldset key={branch.id} className="rounded-2xl border border-slate-200 p-4">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                <label className="text-xs font-bold text-slate-700">Tên chi nhánh<input className={inputClass} value={branch.name} onChange={(event) => updateBranch(branch.id, 'name', event.target.value)} /></label>
                <label className="text-xs font-bold text-slate-700">Địa chỉ<input className={inputClass} value={branch.address} onChange={(event) => updateBranch(branch.id, 'address', event.target.value)} /></label>
                <label className="text-xs font-bold text-slate-700">Điện thoại<input className={inputClass} value={branch.phone} onChange={(event) => updateBranch(branch.id, 'phone', event.target.value)} /></label>
                <label className="text-xs font-bold text-slate-700">Email<input type="email" className={inputClass} value={branch.email} onChange={(event) => updateBranch(branch.id, 'email', event.target.value)} /></label>
                <label className="text-xs font-bold text-slate-700">Dịch vụ, cách nhau bằng dấu phẩy<input className={inputClass} value={branch.services.join(', ')} onChange={(event) => updateBranch(branch.id, 'services', event.target.value.split(',').map((item) => item.trim()).filter(Boolean))} /></label>
                <label className="text-xs font-bold text-slate-700">URL đăng ký<input type="url" className={inputClass} value={branch.ctaUrl} onChange={(event) => updateBranch(branch.id, 'ctaUrl', event.target.value)} /></label>
              </div>
      ...
      <section className="rounded-3xl border border-[#0879D9]/15 bg-white p-5 sm:p-6">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
          <label className="text-sm font-bold text-slate-700">Tiêu đề footer<input className={inputClass} value={draft.footerSettings.heading} onChange={(event) => updateFooter('heading', event.target.value)} /></label>
          <label className="text-sm font-bold text-slate-700">Nhãn CTA<input className={inputClass} value={draft.footerSettings.ctaLabel} onChange={(event) => updateFooter('ctaLabel', event.target.value)} /></label>
          <label className="text-sm font-bold text-slate-700 md:col-span-2">Mô tả<textarea rows={3} className={inputClass} value={draft.footerSettings.description} onChange={(event) => updateFooter('description', event.target.value)} /></label>
          <label className="text-sm font-bold text-slate-700 md:col-span-2">URL CTA<input type="url" className={inputClass} value={draft.footerSettings.ctaUrl} onChange={(event) => updateFooter('ctaUrl', event.target.value)} /></label>
